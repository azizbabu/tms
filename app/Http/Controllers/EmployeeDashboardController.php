...
{
    /**
     * Display dashboard of specific employee
     *
    ...
    {
    	// Check whether employee is exists
    	...
    	$pointStatisticsPayload = $this->getPointStatisticsPayload($employee_id);

        $deadlineStatisticsPayload = $this->getDeadlineStatisticsPayload($employee_id);
        ...
        if($financial_year == 'jan-to-dec') {
            ...
            for ($i=1; $i <= $current_month; $i++) { 
                ...
                $chart_year_data .= "{ x: '".Carbon::now()->month($i)->format('M')."', y: ".$total_role_weight.", z: ".$total_earned_point."},";
            }
        }else {
        ...
        return view('home', compact('employee', 'pointStatisticsPayload','deadlineStatisticsPayload','chart_current_month_data', 'chart_year_data', 'chart_task_completed_data', 'earned_point_within_deadline', 'earned_point_passing_deadline'));
    }

    private function getPointStatisticsPayload($employee_id){
    ...
    private function getDeadlineStatisticsPayload($employee_id){
